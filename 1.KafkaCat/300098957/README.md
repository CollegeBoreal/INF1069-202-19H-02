...
```
ksql> CREATE STREAM BUS_EVENTS_WITH_KEY WITH (KAFKA_TOPIC='bus-events', VALUE_FORMAT='AVRO') AS \
SELECT BUS_ID, CAST(ROUTE_ID), TIMESTAMPTOSTRING(TIMESTAMP, 'yyyy-MM-dd  HH:mm:ss'), LAST_STOP \
FROM BUS_EVENTS PARTITION BY ROUTE_ID;
```
